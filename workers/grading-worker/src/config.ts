...
export const workerConfig = {
  ...
  gradingModel: process.env.OPENAI_GRADING_MODEL ?? 'gpt-5.6-luna',
  transcriptionModel: process.env.OPENAI_TRANSCRIPTION_MODEL ?? 'gpt-4o-transcribe',
  audioModel: process.env.OPENAI_AUDIO_MODEL ?? 'gpt-audio-1.5',
  aiMaxRetries: Number(process.env.AI_MAX_RETRIES ?? 3),
  gradingConcurrency: Number(process.env.AI_GRADING_CONCURRENCY ?? 3),
  transcriptionConcurrency: Number(process.env.AI_TRANSCRIPTION_CONCURRENCY ?? 3),
